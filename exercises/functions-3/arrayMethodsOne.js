...
const mixNumsEven = mixedNumbers.filter(evenNum)
console.log(mixNumsEven)
...
// CODE HERE
// const postTaxPrices = (arr) => {
//   return arr.map((element) =>  element * 1.07)
// }
// const taxRate = 0.07;

// const postTaxPrice = prices.map((price) => {
//   const afterTax = price * (1 + taxRate)
//   return afterTax.toFixed(2)
// })

// console.log(postTaxPrices(prices))
...
const postTaxPrice = prices.map(postTax);
...
const totalPop = populations.reduce((total, elem) => {
   return total += elem
  },0 );
...
const myStrongestCb = monstersInYourPocket.filter(isStrong)

console.log(myStrongestCb);
...
const orderTotal = orders.map((elem) => elem.price * (1+elem.tax))
...
const sumBob = purchases.reduce((total, elem) => {
  if(elem.owner === 'Bob') {
     total += elem.price;
  }
  return total
